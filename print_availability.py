...
CALENDAR_IDS = [
    "primary",
    "ep432@cornell.edu",
]  # Google calendars to draw from
MIN_TIME_INTERVAL_LENGTH_IN_MINUTES = (
    30  # don't print out availabilities shorter than this
)
...
CREDENTIALS_NAME_DOWNLOADED_FROM_GOOGLE = (
    "google_calendar_credentials.json"  # filename you use for credentials.
)
...
if len(sys.argv) == 3:
    ...
    START_DATETIME = tz.localize(
        datetime.datetime(*start_datestring, START_HOUR)
    )
    ...
    START_DATETIME = tz.localize(
        datetime.datetime(today.year, today.month, today.day, START_HOUR)
    )
    END_DATETIME = tz.localize(
        datetime.datetime(
            two_weeks_later.year,
            two_weeks_later.month,
            two_weeks_later.day,
            END_HOUR,
        )
    )
...
print(
    "*****Printing availabilities between %s and %s of at least %i minutes"
    % (
        "-".join(str(START_DATETIME).split("-")[:-1]),
        "-".join(str(END_DATETIME).split("-")[:-1]),
        MIN_TIME_INTERVAL_LENGTH_IN_MINUTES,
    )
)
print("Drawing from calendars with IDs", CALENDAR_IDS, "using timezone", tz)
print(
    "Feel free to change all these options by changing the relevant variables at the top of the script"
)
...
def main():
    ...
    if os.path.exists(os.path.join(BASE_PATH, "token.json")):
        creds = Credentials.from_authorized_user_file(
            os.path.join(BASE_PATH, "token.json"), SCOPES
        )
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                os.path.join(
                    BASE_PATH, CREDENTIALS_NAME_DOWNLOADED_FROM_GOOGLE
                ),
                SCOPES,
            )
    ...
    try:
            ...
            unified_busy_list.append(
                {
                    "start": datetime.datetime.strptime(
                        all_events[i]["start"]["dateTime"], parse_string
                    ),
                    "end": datetime.datetime.strptime(
                        all_events[i]["end"]["dateTime"], parse_string
                    ),
                }
            )
        ...
        while current_time < END_DATETIME:
            if (
                current_time.hour == END_HOUR
            ):  # at the end of the day, go to the next day.
                current_time = (
                    current_time
                    + datetime.timedelta(days=1)
                    + datetime.timedelta(hours=START_HOUR - END_HOUR)
                )
            while current_time.strftime("%A") in [
                "Saturday",
                "Sunday",
            ]:  # skip weekends.
                current_time = current_time + datetime.timedelta(days=1)
            all_potential_free_intervals.append(
        ...
        for interval_start, interval_end in all_potential_free_intervals:
            ...
            for busy_interval in unified_busy_list:
                ...
                no_overlap = (interval_end <= busy_interval["start"]) or (
                    interval_start >= busy_interval["end"]
                )
                if not no_overlap:
                    is_free = False
                    break
        ...
        assert (
            sorted(confirmed_free_intervals, key=lambda x: x[0])
            == confirmed_free_intervals
        )
        ...
        for next_interval in confirmed_free_intervals[1:]:
            if next_interval[0] == merged_end:
                ...
                if (merged_end - merged_start).seconds >= (
                    MIN_TIME_INTERVAL_LENGTH_IN_MINUTES * 60
                ):
        ...
        if (merged_end - merged_start).seconds >= (
            ...
            )  # don't forget to add last interval
        ...
        for interval in merged_intervals:
            ...
            intervals_by_date[date].append(
                interval[0].strftime("%-I:%M")
                + "-"
                + interval[1].strftime("%-I:%M")
            )
        ...
        for date in sorted(intervals_by_date.keys()):
            ...
            if len(intervals_by_date[date]) == 1:
                ...
                if len(intervals_by_date[date]) == 2:
                    ...
                    comma_string = ", or "
                timestrings = (
                    ", ".join(intervals_by_date[date][:-1])
                    + comma_string
                    + intervals_by_date[date][-1]
                )
